...
# Define the local directory for error videos
VIDEO_DIR="./error_videos"
...
generate_error_video() {
    ...
    # Write message to a temporary file, interpreting backslash escapes
    echo -e "$message" > "$text_file"

    ffmpeg -f lavfi -i color=c=black:s=1920x1080:d=10 -vf "drawtext=fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf:textfile='$text_file':fontsize=72:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2" -c:v libx264 -t 10 -pix_fmt yuv420p -movflags +faststart -y "$output"
